...
Кнопка **Применить все рекомендации MEKO** выполняет комплексную настройку:
...
- задаёт `max_connections=16384`, `client_handshake=15`,
  `tg_connect=30`, `client_keepalive=120`;
- пересоздаёт контейнеры Telemt с обновлённой конфигурацией.

### Редактор config.toml

На карточке каждого прокси доступна кнопка **Config** и пункт меню
**Конфигурация Telemt**. Редактор загружает фактический
`/etc/telemt/config.toml` из контейнера и позволяет сохранить изменения.
Сервис-нода проверяет обязательные секции, текущий порт и секрет, сохраняет
override в `store.json` и пересоздаёт контейнер. Если Telemt завершится после
изменения, предыдущая конфигурация будет автоматически восстановлена.

Структурированное редактирование прокси через обычную форму снова генерирует
конфигурацию из полей панели и отключает ручной override.
